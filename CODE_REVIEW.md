...
| `manifest.py` | `RunManifest`/`build_manifest()` — resolved config, config hash, git commit + dirty-tree flag, env hash, hardware, start/end time, GPU-hours, `nondeterministic_ops`. Written to `artifacts/runs/<run_id>/manifest.json`. |
